...
def _pedigree_item(rec: bytes, idx: int) -> tuple[str, str]:
    ...
    base=205 と stride=46 を拘束する。idx 8 / 12 はその配列算術による導出だが
    **暫定確定**: offset/stride のずれは馬名の文字化けで可視になる一方、gen3 内の
    順列取り違え (例: 父父母 ↔ 父母父) は妥当な馬名が表示されるため**不可視**。
    実機での血統表突合 (docs/OPERATION.md「3代血統・産地の検証」) までは暫定扱い。
    """
    pos = 205 + idx * 46
...
def parse_hn(rec: bytes) -> BreedingHorse:
    ...
    230 (父繁殖番号) の間隙 25 byte に適合するが、**間隙適合は必要条件にすぎず
    暫定確定** (順序は JV-Data 標準構造体の知識に依拠)。特に持込区分 (1 桁数字) ↔
    輸入年 (4 桁数字) の内部順序の誤りは数字同士のため**無音で混入**する。
    実機確認手順 (産地名の目視 / 数字混入チェック / クロス整合) は
    docs/OPERATION.md「3代血統・産地の検証」参照。確認完了までこの 3 フィールドは
    未検証データとして扱うこと。
    """
