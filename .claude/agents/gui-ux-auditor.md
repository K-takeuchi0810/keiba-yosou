...
本 agent の評価は **type-D (GUI/HTML 表示) の改修に適用**。`gui/app.py` に変更が無い type-A/B/C 改修
では、(a) GUI 回帰の有無を `git show HEAD --stat` で確認 (無変更なら前回スコア維持) し、(b) 当該改修の
出力を将来 GUI に露出する際の UX 留意点のみ **参考所見**として簡潔に述べる。GUI 無変更を理由に
NOT_EVALUABLE ではなく「前回維持」とする。P25 固有 (fresh/stale counts 表示・補正 ON/発火区別) は
GUI に当該表示が関わる type-A/D のときのみ Hard Fail として扱う。JS パース確認は CONTROL_HTML 変更時のみ必須。
