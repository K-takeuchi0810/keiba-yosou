...
本 agent の評価は **type-D (web/templates・generator が出力する HTML) の改修に適用**。`web/` に変更が
無い type-A/B/C 改修では、(a) HTML 回帰の有無を `git show HEAD --stat` で確認 (無変更なら前回スコア維持)、
(b) 当該改修の出力を将来スマホ HTML に出す際の留意点のみ **参考所見**として簡潔に述べる。web 無変更を
理由に NOT_EVALUABLE ではなく「前回維持」とする。P25 固有 (fresh/stale 表示・誤読防止) は HTML に
当該表示が関わる type-A/D のときのみ Hard Fail として扱う。`web/dist` 再生成必須も type-D のときのみ。
