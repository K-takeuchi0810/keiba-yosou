...
**扱い (2026-09-26 CHAT 決定)**

- 同じ秒の上書き (主キーに `source` が無い) は **DATA_PROVENANCE_DEFECT /
  REQUIRES_FIX_BEFORE_PIT_DATASET_FREEZE**。オッズの値を壊すのではなく、「その取得秒に
  どの取得元の観測があったか」を後から完全には再現できないことが問題。JST の凍結中は
  schema / ingest を触らず記録だけ。直すときは、主キーに `source` を足すだけでよいかを含め、
  migration・既存データ・読み手をまとめてレビューする
- ③ は 4A/4B を上書きしない別の成果物で、次の系列を並べる:
  `original_mixed` (DB のまま、4A/4B の再現の基準) / `raw_mixed` (raw から組み直した混合、
  上書きの影響を除く) / `raw_0B30` / `raw_0B31` / `matched_same_state`
- raw からの再構築でも `fetched_at <= cutoff` を絶対条件にする。取得時刻の由来 (ファイル名の
  epoch か mtime か) を manifest に残す
- 一致の分類: `matched_same_state` (同じレース・同じ発表時刻・単勝票数合計が一致し、raw 上で
