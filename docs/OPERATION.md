...
### 9-2. バイト位置の実機検証 (状態: UM=確定済み / HN=オフセット誤り判明)

**UM idx8/idx12 (父母父/母母父) は 2026-07-06 実 DB で確定済み** — ディープ産駒 6 頭
すべての父母父 = Alzao と一致、充填 89-94% (scripts/verify_pedigree.py の出力)。

**HN 205-229 (持込区分/輸入年/産地名) はオフセット誤りが判明** — 産地が先頭 1 文字
欠け + 末尾に隣接繁殖番号 "11" 混入。webapp は config.HN_BIRTHPLACE_VERIFIED=False で
表示抑制中。正しい位置を突き止めて修正する手順:

- `python -m scripts.probe_hn_offsets` で生バイト (195-250 + 繁殖番号 228-249) を
  複数候補幅で表示 → 産地名が全角で綺麗に読める開始位置を特定。
- **230-239/240-249 の繁殖番号も同方向シフトの疑い** (数字同士で無音誤り)。probe の
  「父繁殖/母繁殖 現行 vs 候補」行で 10 桁数字が綺麗に出る位置を確認。
- `jvlink_client/parser.py` の parse_hn の 205/206/210/230/240 を修正 →
  `ingest_all(force=True, dataspecs=['BLOD'])` で再取込 →
  下記の目視検証を通過したら config.HN_BIRTHPLACE_VERIFIED=True に反転。
...
5. **11 大系統外の 3 系統 (2026-07-06 追加)** — personon (パーソロン系: シンボリルドルフ/
