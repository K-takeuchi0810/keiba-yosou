...
1. コンソール / タスクログに `notify suppressed (generation_complete:YYYYMMDD): duplicate`
   が出ていれば、抑止であって失敗ではない。
...
評価処理に手を入れてはいけない。導入時の確認:

- 同日 HTML を 変更後 → 変更前 → 変更後 の順に連続生成し、3 本とも生成時刻の
  1 行を除いて sha256 一致 (`c0782ff493fc0eba`)。同一コードを時間差で 2 回
  回す対照を挟んでいるので、「たまたま一致した」ではない。
- `prediction_log` 18,893 行の sha256 が前後で一致。
- 封印モデル成果物 6 点のハッシュ不変。
- `web.generator` のプロセスが `scripts.auto_predict` / `scripts.notify_dedup`
  を 1 つも import しないことを実測。

**注意**: オッズは live で動くので、時間を空けた 2 本の HTML は当然ずれる
(14 分空けたら 432 ハンクずれた)。前後比較は必ず連続で回すこと。
