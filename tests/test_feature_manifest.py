...
def test_conditional_logit_recovers_a_known_coefficient():
    """条件付きロジットが既知の係数を復元すること。

    2026-09-19 に発散を実測した (logit(P_T10) の 3 次項と補正でスケールが
    4 桁違い、係数が −2.7e9 になった)。数値が壊れていないことを固定する。
    """
    import random as _r

    import numpy as np

    from scripts.fundamental_eval import conditional_logit
    ...
    for i in range(4000):
        ...
        rows = []
        for j in range(n):
            # わざと 4 桁スケールが違う 2 列にする (壊れた条件を再現)
            rows.append({"race_id": f"r{i}", "won": 0,
                         "big": rng.gauss(0, 100.0), "small": rng.gauss(0, 0.01)})
        u = np.array([true_beta[0] * r["big"] / 100.0
                      + true_beta[1] * r["small"] / 0.01 for r in rows])
        u -= u.max()
        w = np.exp(u); w /= w.sum()
        rows[list(np.cumsum(w) >= rng.random()).index(True)]["won"] = 1
        samples.extend(rows)

    beta = conditional_logit(samples, ["big", "small"])

    # スケールを戻すと true_beta / scale になる
    assert abs(beta[0] * 100.0 - true_beta[0]) < 0.12, beta
    assert abs(beta[1] * 0.01 - true_beta[1]) < 0.12, beta


def test_conditional_logit_does_not_explode_on_collinear_columns():
    """共線に近い列を入れても爆発しないこと (発散の回帰テスト)。"""
    import random as _r
    ...
    for i in range(600):
        ...
        for j in range(n):
            z = rng.gauss(-2, 1.5)
            samples.append({"race_id": f"r{i}", "won": 1 if j == 0 else 0,
                            "z": z, "z2": z * z, "z3": z ** 3,
                            "tiny": rng.gauss(0, 0.05)})

    beta = conditional_logit(samples, ["z", "z2", "z3", "tiny"])

    assert all(abs(b) < 1e4 for b in beta), f"係数が爆発している: {beta}"
