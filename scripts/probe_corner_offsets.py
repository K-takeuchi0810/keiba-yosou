...
検証ロジック:
  ...
  3. サニティ:
     - corner_order_4 は 1..出走頭数 の範囲に収まるはず (順位なので)。
     - 同一レース内で corner_order_4 の値がほぼ一意 (順位なので重複が少ない)。
     - 先行馬 (逃げ/先行) の corner_order_4 は小さい傾向。
  4. 逸脱 (範囲外・全部同値・全部0) があれば「offset がズレている可能性大」と警告。
