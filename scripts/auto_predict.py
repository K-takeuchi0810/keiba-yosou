...
REPO_URL = "https://github.com/K-takeuchi0810/keiba-yosou"
...
def main() -> int:
    ...
    # marker commit + push (案A: commit URL を通知に載せる)
    MARKER.parent.mkdir(parents=True, exist_ok=True)
    from datetime import datetime
    MARKER.write_text(
        f"# 最新予想生成\n\n- 対象: {d_from}〜{d_to} ({n_races} レース)\n"
        f"- 生成時刻: {datetime.now().isoformat(timespec='seconds')}\n"
        f"- モデル: {ver}\n- 閲覧: iCloud Drive の index.html (iPhone)\n",
        encoding="utf-8")
    subprocess.run(["git", "add", str(MARKER)], cwd=PROJECT_ROOT, check=True)
    c = subprocess.run(["git", "commit", "-m",
                        f"predictions: {d_from}-{d_to} generated ({ver})"],
    ...
    _notify(
        ...
        f"({n_races}R, {ver})\n"
        f"📄 閲覧: iCloud Drive → 競馬予想 → index.html\n"
        f"🔗 {REPO_URL}/commit/{sha}\n"
