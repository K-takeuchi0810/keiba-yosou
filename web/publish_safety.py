...
def assess_race_completeness(
    ...
    *,
    ...
    for day in days:
        ...
        if day_key in target_dates:
            relevant_races.extend(day.get("races") or [])
    total = len(relevant_races)
    empty = sum(not (race.get("horses") or []) for race in relevant_races)
    ratio = empty / total if total else 0.0
    return {
        "total_races": total,
        "empty_races": empty,
        "empty_race_ratio": ratio,
        "threshold": threshold,
        "alert": total > 0 and ratio > threshold,
    }
